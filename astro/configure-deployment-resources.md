...
For a list of supported node instance types and their corresponding worker size limits, see cloud resource references ([AWS](resource-reference-aws.md#deployment-worker-size-limits), [Azure](resource-reference-azure.md#deployment-worker-size-limits), [GCP](resource-reference-gcp.md#deployment-worker-size-limits)). To request a different instance type for your cluster, contact [Astronomer support](https://support.astronomer.io).
