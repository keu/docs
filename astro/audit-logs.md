...
2. In **Audit Logs**, select the number days of audit data to export and then click **Export**.
    ...
    The extracted audit log data is saved as a [newline delimited JSON (ndjson)](https://github.com/ndjson/ndjson-spec) file in your `downloads` directory with the default filename `<astro-organization-name>-logs-<number-of-days>-days-<date>.ndjson`.
