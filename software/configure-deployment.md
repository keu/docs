...
You can run a cron job to automatically archive task and DAG metadata from your Deployment. This job runs [`airflow db clean`](https://airflow.apache.org/docs/apache-airflow/stable/cli-and-env-variables-ref.html#clean) for all of your Deployments and exports the results as a file to a configured external storage service. 

Task usage metrics are generated by the Astronomer-maintained `plugin-name`. Deployments using Runtime 8.0 or later have this plug-in installed by default.

To clean task metadata from your Deployments:

1. If your Deployment is on Runtime 7 or earlier, add the following line to the `requirements.txt` file of your Astro project:

    ```text
    https://github.com/astronomer/[LINK NEEDED]
    ```
