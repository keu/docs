...
> **Note:** You can also use the  Astro CLI to configure NFS volumes. To do so, specify the `--nfs-location` flag when running [`astro deployment create`](https://docs.astronomer.io/astro/cli/astro-deployment-create?tab=software) or [`astro deployment update`](https://docs.astronomer.io/astro/cli/astro-deployment-update?tab=software).
